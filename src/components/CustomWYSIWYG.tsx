...
const highlightMarkdown = (text: string, activeLineIndex: number = -1, isZenMode: boolean = false) => {
  ...
  const highlightedLines = lines.map((line, index) => {
    ...
    if (/^:::test$/.test(line)) {
      inTestBlock = true;
      return `<span class="md-line${zenClass} block w-full bg-indigo-50/70 dark:bg-indigo-900/30 border-l-[3px] border-indigo-500 font-mono text-indigo-400 opacity-70 text-[11px] uppercase tracking-widest pl-2 pt-1"><span class="md-token">:::test</span></span>`;
    }
    if (/^:::$/.test(line) && inTestBlock) {
      inTestBlock = false;
      return `<span class="md-line${zenClass} block w-full bg-indigo-50/70 dark:bg-indigo-900/30 border-l-[3px] border-indigo-500 font-mono text-indigo-400 opacity-70 text-[11px] uppercase tracking-widest pl-2 pb-1 rounded-bl"><span class="md-token">:::</span></span>`;
    }
    
    if (inTestBlock) {
      if (line.match(/^-\s+\[[ x]\]/i)) {
        zenClass += " block w-full bg-indigo-50/70 dark:bg-indigo-900/30 border-l-[3px] border-indigo-500 pl-2 text-slate-700 dark:text-slate-300";
      } else {
